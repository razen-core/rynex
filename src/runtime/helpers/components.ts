...
export function avatarLegacy(
  props: DOMProps & { src: string; alt?: string; size?: string | number },
): HTMLElement {
...
export function iconLegacy(
  props: DOMProps & { size?: string | number },
  svgContent: string,
): HTMLElement {
...
export function modalLegacy(
  props: DOMProps & { open?: boolean; onClose?: () => void },
  ...children: DOMChildren[]
): HTMLElement {
  ...
  build(): HTMLDivElement {
    ...
    this.items.forEach((item, index) => {
      ...
      Object.assign(itemEl.style, {
        ...
        cursor: "pointer",
        borderBottom:
          index < this.items.length - 1 ? "1px solid #333333" : "none",
...
export function dropdownLegacy(
  props: DOMProps & { items: Array<{ label: string; onClick: () => void }> },
  trigger: DOMChildren,
): HTMLElement {
  ...
  tabsData.forEach((tab, index) => {
    const header = createElement("button", {
      ...
      style: {
        ...
        border: "none",
        borderBottom:
          index === activeIndex ? "2px solid #00ff88" : "2px solid transparent",
  ...
  build(): HTMLDivElement {
      ...
      if (isOpen) {
        ...
        if (!this.allowMultiple) {
          ...
          Array.from(this.element.children).forEach((child) => {
            ...
            const itemIcon = child.querySelector(
              ".accordion-icon",
            ) as HTMLElement;
            if (content && itemIcon) {
    ...
    this.items.forEach((item, index) => {
      ...
      Object.assign(icon.style, {
        ...
        transform: this.openIndices.has(index)
          ? "rotate(180deg)"
          : "rotate(0deg)",
      });
      icon.textContent = "▼";
