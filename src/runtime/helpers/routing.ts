...
export function RouterOutlet(router: Router): HTMLDivElement {
  const outlet = createElement('div', { class: 'router-outlet' }) as HTMLDivElement;
  router.mount(outlet);
  return outlet;
}
...
export function RouteGuard(
  ...
  const container = createElement('div', { class: 'route-guard' }) as HTMLDivElement;
...
export function Breadcrumb(
  ...
  const updateBreadcrumb = () => {
    ...
    paths.forEach((segment, index) => {
      ...
      const isLast = index === paths.length - 1;
      if (isLast) {
        const span = createElement('span', { class: 'breadcrumb-item active' }) as HTMLSpanElement;
        span.textContent = segment;
