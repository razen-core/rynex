...
export function formatError(error: Error, context?: ErrorContext): string {
  ...
  if (context?.file) {
    ...
    if (context.line !== undefined) {
      lines.push(`Location: Line ${context.line}${context.column ? `, Column ${context.column}` : ''}`);
    }
  }
...
const errorSuggestions: Record<string, string> = {
  ...
  'EEXIST': 'File or directory already exists. Use a different name or remove the existing one.',
  'MODULE_NOT_FOUND': 'Module not found. Run `npm install` to install dependencies.',
  'SyntaxError': 'Syntax error in your code. Check for typos, missing brackets, or incorrect syntax.',
  'TypeError': 'Type error. Check if you\'re using the correct data types.',
  'ReferenceError': 'Variable not defined. Make sure to declare variables before using them.',
};
