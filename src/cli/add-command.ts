 ...
 */
async function promptPackageManager(): Promise<
  "npm" | "pnpm" | "yarn" | "bun"
> {
...
function ensureCSSImport(projectRoot: string): void {
  ...
  for (const entryPath of entryPoints) {
    if (fs.existsSync(entryPath)) {
      ...
      if (
        ...
        content.includes("/public/styles.css") ||
        content.includes("bundle.css")
      ) {
  ...
  logger.info(
    'Please manually add: import "./public/styles.css"; to your entry file',
  );
}
...
function addCSSLinkToHTML(projectRoot: string): void {
  ...
  logger.info(
    'Please manually add: <link rel="stylesheet" href="./bundle.css"> to your HTML <head>',
  );
}
...
async function addTailwind(): Promise<void> {
  ...
  try {
    ...
    logger.info(
      '  UI.container({ class: "flex items-center justify-center" })',
    );
  } catch (error) {
...
export async function handleAddCommand(integration: string): Promise<void> {
  ...
  if (
    normalizedIntegration === "tailwind" ||
    normalizedIntegration === "tailwindcss"
  ) {
    await addTailwind();
  }
}
