  ...
  createElement: {
    ...
    rules: [
      ...
      },
      {
        argIndex: 1,
        type: 'object',
        optionalProps: ['class', 'id', 'style', 'onClick', 'onChange', 'onHover', 'onInput', 'onFocus', 'onBlur'],
    ...
    rules: [
      {
        argIndex: 0,
        type: 'object',
        optionalProps: ['style', 'class', 'onClick', 'onHover'],
        message: 'card() expects props object',
        severity: 'warning'
      }
    ]
  },
    ...
    rules: [
      {
        argIndex: 0,
        type: 'object',
        optionalProps: ['open', 'onClose', 'style', 'class', 'onClick', 'onHover'],
        message: 'modal() expects props object',
        severity: 'warning'
      }
    ]
  },
    ...
    rules: [
      {
        ...
        type: 'object',
        requiredProps: ['tabs'],
        optionalProps: ['defaultIndex', 'onChange'],
        message: 'tabs() requires tabs property',
        severity: 'error'
      }
    ]
  },
    ...
    rules: [
      {
        ...
        type: 'object',
        requiredProps: ['items'],
        optionalProps: ['allowMultiple', 'defaultOpen'],
        message: 'accordion() requires items property',
        severity: 'error'
      }
    ]
  },
...
export function validateRynexCode(projectRoot: string, config: Partial<ValidatorConfig> = {}): ValidationError[] {
...
function validateFile(filePath: string, context: ValidationContext): ValidationError[] {
    ...
    function visit(node: ts.Node) {
      if (ts.isCallExpression(node)) {
        ...
        if (functionName && RYNEX_VALIDATION_RULES[functionName as keyof typeof RYNEX_VALIDATION_RULES]) {
          const error = validateRynexFunctionCall(node, functionName, filePath, sourceFile, context);
          if (error) {
            errors.push(error);
...
function getArgumentType(arg: ts.Expression): string {
  ...
  if (ts.isFunctionExpression(arg) || ts.isArrowFunction(arg)) return 'function';
...
function validateRynexFunctionCall(
  ...
  const codeSnippet = sourceFile.text.substring(node.getStart(), Math.min(node.getEnd(), node.getStart() + 100));
  ...
  for (const rule of rules.rules) {
    ...
    const { line: argLine, character: argChar } = sourceFile.getLineAndCharacterOfPosition(arg.getStart());
    ...
    if (rule.type === 'object' && ts.isObjectLiteralExpression(arg)) {
      ...
      if (context.config.checkOptionalProps && rule.optionalProps) {
        const unusedProps = props.filter(p => 
          !rule.requiredProps?.includes(p) && !rule.optionalProps?.includes(p)
        );
    ...
    if (rule.type === 'function' && !ts.isFunctionExpression(arg) && !ts.isArrowFunction(arg) && !ts.isIdentifier(arg)) {
      return {
        file: filePath,
        line: argLine + 1,
        column: argChar + 1,
    ...
    if (rule.type === 'number' && !ts.isNumericLiteral(arg) && !ts.isIdentifier(arg)) {
      return {
        file: filePath,
        line: argLine + 1,
        column: argChar + 1,
...
function generateValidationReport(errors: ValidationError[], duration: number): ValidationReport {
...
export function printValidationErrors(errors: ValidationError[], verbose: boolean = false): void {
  if (errors.length === 0) {
  ...
  for (const [file, fileErrors] of Object.entries(errorsByFile)) {
    ...
    for (const error of fileErrors) {
      ...
      console.log(`    ${colors.gray}${error.line}:${error.column}${colors.reset} ${colors.bold}[${severityColor}${severityLabel}${colors.reset}${colors.bold}]${colors.reset} ${error.message}`);
      
      // Show code location
      if (error.code) {
        const codePreview = error.code.substring(0, 60).replace(/\n/g, ' ');
        console.log(`      ${colors.gray}→ ${codePreview}${error.code.length > 60 ? '...' : ''}${colors.reset}`);
      }
      
      if (error.suggestion && verbose) {
        console.log(`      ${colors.green}💡 Suggestion: ${error.suggestion}${colors.reset}`);
      }
...
export function printDetailedReport(errors: ValidationError[]): void {
  ...
  console.log(`  Total Issues: ${report.totalErrors + report.totalWarnings + report.totalInfos}`);
...
export function getValidationStats(projectRoot: string, config: Partial<ValidatorConfig> = {}): any {
  const finalConfig = { ...DEFAULT_CONFIG, ...config };
  const context: ValidationContext = {
    config: finalConfig,
    cache: validationCache,
...
export function runRynexValidation(projectRoot: string, options: { verbose?: boolean; detailed?: boolean; config?: Partial<ValidatorConfig> } = {}): boolean {
  const startTime = Date.now();
