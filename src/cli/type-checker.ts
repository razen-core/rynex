...
function printDiagnostic(diagnostic: ts.Diagnostic): void {
  if (diagnostic.file && diagnostic.start !== undefined) {
    ...
    if (
      ...
      const lineStart = diagnostic.file.getLineAndCharacterOfPosition(
        diagnostic.start,
      ).line;
      ...
      for (let i = startLine; i <= endLine; i++) {
        ...
        if (i === lineStart) {
          const col = diagnostic.file.getLineAndCharacterOfPosition(
            diagnostic.start!,
          ).character;
          const underline =
            " ".repeat(col + 10) + "^".repeat(Math.min(diagnostic.length!, 50));
