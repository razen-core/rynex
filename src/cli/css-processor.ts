...
export async function processCSS(
  ...
  try {
    ...
    if (postcss) {
      return await processWithPostCSS(
        postcss,
        entryPath,
        outputPath,
        minify,
...
async function processWithCLI(
  ...
  return new Promise((resolve) => {
    const args = [entryPath, "-o", outputPath];
...
export function watchCSS(
  ...
  const args = [entryPath, "-o", outputPath, "--watch"];
  ...
  if (!fs.existsSync(postcssPath)) {
    logger.error(
      "PostCSS CLI not found. Install with: pnpm add -D postcss-cli",
    );
    return null;
  }
